    ...
    stage("Running Bootstrap data Operation env-name= $ENVIRONMENT_NAME") {
        ...
        dir('nsl-infra'){
            ...
            if (ENVIRONMENT_NAME) {
                ...
                sh "sed -ie 's/.*instance_filters = tag:env=.*\$/instance_filters = tag:env=$env_instance_name/g' aws_utils/ec2.ini && ansible-playbook $verbose -i aws_utils/ec2.py -u ubuntu playbooks/bootstrap_db.yml --tags \"load-data\" -e $extra_vars --extra-vars $shard_vars > /tmp/bootstrap-db.log"
