    ...
    stage("Deploy services to $ENVIRONMENT_NAME") {
        dir('nsl-infra') {
            ...
            if (ENVIRONMENT_NAME) {
                ...
                sh "sed -ie 's/.*instance_filters = tag:env=.*\$/instance_filters = tag:env=$env_instance_name/g' aws_utils/ec2.ini && ansible-playbook  -i aws_utils/ec2.py -u ubuntu playbooks/deploy-rds.yml -e $extra_vars --extra-vars $shard_vars"
                ...
                sh "ansible-playbook  -i inventory/$env_name -u ubuntu playbooks/deploy-rds.yml -e $extra_vars --extra-vars $shard_vars"
            }
    ...
    stage("Deploy editor to $ENVIRONMENT_NAME") {
        ...
        dir('nsl-infra') {
            if (ENVIRONMENT_NAME) {
                ...
                sh "sed -ie 's/.*instance_filters = tag:env=.*\$/instance_filters = tag:env=$ENVIRONMENT_NAME/g' aws_utils/ec2.ini && ansible-playbook  -i aws_utils/ec2.py -u ubuntu playbooks/deploy-rds.yml -e $extra_vars -e $shard_vars"
            } else if (INVENTORY_NAME) {
                def extra_vars = /'{"nxl_env_name":"$INVENTORY_NAME","apps":[{"app": "editor"}], "war_names": [{"war_name": "nxl#editor##1.53"}   ],   "war_source_dir": "$projectDir"}'/
                sh "ansible-playbook -vvv -i inventory/$INVENTORY_NAME -u ubuntu playbooks/deploy-rds.yml -e $extra_vars -e $shard_vars"
            }
        }
    }
